...
function bi.mobdb.Match()
    for i = 1, table.getn(bi.mobdb.mobs) do
        if bi.mobdb.mobs[i].Name == bi.api.TargetName then
            return true
        end
    end
    return false
...
-- function bi.mobdb.MatchIntSpell()
--     if not UnitExists('target') then
--         return false
--     end
--     for i = 1, table.getn(bi.mobdb.mobs) do
--         if bi.mobdb.mobs[i].name == bi.api.TargetName and (bi.mobdb.mobs[i].intSpell1 == bi.api.EnemySpellcast or bi.mobdb.mobs[i].intSpell2 == bi.api.EnemySpellcast) then
--             return true
--         end
--     end
--     return false
...
function bi.mobdb.Casts()
	if not UnitExists('target') or not Cg("MobDBFilter") then
        return false
    end
    for i = 1, table.getn(bi.mobdb.mobs) do
        if bi.mobdb.mobs[i].Name == bi.api.TargetName then
			if not bi.mobdb.mobs[i].IntSpell then
				bi.log.Warn("Bad IntSpell table in bi.mobdb.mobs[i].Name")
				return false
			end
			return (bi.api.Tablelength(bi.mobdb.mobs[i].IntSpell) > 0)
        end
    end
    return false
end
...
function bi.mobdb.MatchIntSpell()
    ...
    for i = 1, table.getn(bi.mobdb.mobs) do
        if bi.mobdb.mobs[i].Name == bi.api.TargetName then
            ...
            --print(bi.api.TargetName .. " is a MobDB match!")
            for _, v in pairs(bi.mobdb.mobs[i].IntSpell) do
                if v == bi.api.EnemySpellcast then
                    bi.log.Debug(bi.api.EnemySpellcast .. " = " .. v .. " : Must Interrupt")
                    return true
                else
                    --print(v .. " did not match ".. bi.api.EnemySpellcast .. ".")
                end
            end
        end
    end
...
function bi.mobdb.MatchIgnSpell()
    if not UnitExists('target') or not Cg("MobDBFilter") then
        return false
    end
    for i = 1, table.getn(bi.mobdb.mobs) do
        if bi.mobdb.mobs[i].Name == bi.api.TargetName then
			if not bi.mobdb.mobs[i].IgnSpell then
				bi.log.Warn("Bad IgnSpell table in bi.mobdb.mobs[i].Name")
				return false
			end
            --print(bi.api.TargetName .. " is a MobDB match!")
            for _, v in pairs(bi.mobdb.mobs[i].IgnSpell) do
                if v == bi.api.EnemySpellcast then
                    bi.log.Debug(bi.api.EnemySpellcast .. " = " .. v .. " : Do Not Interrupt")
                    return true
                else
                    --print(v .. " did not match ".. bi.api.EnemySpellcast .. ".")
                end
...
function bi.mobdb.MatchGlobalIgnSpell()
	if not UnitExists('target') or not Cg("IntSpellCatchAll") then
        return false
    end

	for _, v in pairs(bi.mobdb.globalignspell) do
		--print("globalignspell evaluating " .. v)

		if v == bi.api.EnemySpellcast then
			bi.log.Debug(bi.api.EnemySpellcast .. " = " .. v .. " : Global Do Not Interrupt")
			return true
		else
			--print(v .. " did not match ".. bi.api.EnemySpellcast .. ".")
		end
	...
	--bi.log.Debug(bi.api.EnemySpellcast .. " : DOES NOT MATCH Global Do Not Interrupt")
